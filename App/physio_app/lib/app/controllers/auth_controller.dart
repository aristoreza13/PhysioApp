import 'package:physio_app/app/data/models/users_model.dart';
...
class AuthController extends GetxController {
  ...
  //agar bisa dipantau setiap perubahan
  var user = UsersModel().obs;

  // mengakses firebase firestore
  FirebaseFirestore firestore = FirebaseFirestore.instance;

  // inisialisasi aplikasi pertama kali ketika fresh install
  ...
  void addNewConnection(String doctorEmail) async {
    ...
    if (docChats.docs.length != 0) {
      ...
      final checkConnection = await users
          ...
          .collection("chats")
          .where("connection", isEqualTo: doctorEmail)
          .get();

      if (checkConnection.docs.length != 0) {
        // sudah pernah buat koneksi dengan => doctorEmail
        flagNewConnection = false;

        //chat_id from chats collection
        chat_id = checkConnection.docs[0].id;
      } else {
        // blm pernah buat koneksi dengan => doctorEmail
        // buat koneksi ....
  ...
  // FUNGSI UNTUK TERAPIS (BETA TESTING)

  // LOGIN SEBAGAI TERAPIS
  Future<void> loginTerapis() async {
    try {
      // Ini untuk handle kebocoran data user sebelum login
      await _googleSignIn.signOut();
